...
| **SM 活跃率（SMACT）** | **~56%** | **80%+（峰值100%）** | **+24%** |
| **SM 占用率（SMOCC）** | ~25% | **36~60%** | **大幅提升** |
| **GPU 活跃率（GRACT）** | 96~100% | 96~100% | 相同 |
...
#   0   55    7   ← 计算间隙，SM 空转
#   0  100   11   ← 计算峰值
#   0   33    5   ← checkpoint 保存期间
# 平均约 56%

# 优化训练（batch=8）典型输出：
# gpu   sm  mem
#   0  100   36   ← 计算峰值，内存带宽也提升
#   0  100   58   ← 持续高利用率
#   0   85   50   ← 空闲期也维持较高水平
# 平均约 80%+
```
